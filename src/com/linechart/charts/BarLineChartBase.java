...
import com.linechart.components.Legend.LegendPosition;
import com.linechart.components.XAxis;
import com.linechart.components.XAxis.XAxisPosition;
import com.linechart.components.YAxis;
import com.linechart.components.YAxis.AxisDependency;
import com.linechart.data.BarLineScatterCandleData;
import com.linechart.data.BarLineScatterCandleDataSet;
...
import com.linechart.data.filter.Approximator;
import com.linechart.interfaces.BarLineScatterCandleDataProvider;
import com.linechart.listener.BarLineChartTouchListener;
import com.linechart.renderer.XAxisRenderer;
...
public abstract class BarLineChartBase<T extends BarLineScatterCandleData<? extends BarLineScatterCandleDataSet<? extends Entry>>>
	...
	protected void prepareValuePxMatrix() {
		this.mLeftAxisTransformer.prepareMatrixValuePx(this.mXChartMin, this.mDeltaX, this.mAxisLeft.mAxisRange,
	...
	public void notifyDataSetChanged() {
		if (!this.mDataNotSet) {
			...
			this.mAxisRendererLeft.computeAxis(this.mAxisLeft.mAxisMinimum, this.mAxisLeft.mAxisMaximum);
			this.mXAxisRenderer.computeAxis(((BarLineScatterCandleData) this.mData).getXValAverageLength(),
					((BarLineScatterCandleData) this.mData).getXVals());
	...
	protected void calcMinMax() {
		...
		if (max == min) {
			...
			this.mAxisLeft.scale = (this.mAxisLeft.mAxisMaximum - this.mAxisLeft.mAxisMinimum)
					/ ((float) this.mAxisLeft.getLabelCount() - 1.0F);
		...
		if (!this.mCustomViewPortEnabled) {
			...
			if (this.mLegend != null && this.mLegend.isEnabled()) {
				if (this.mLegend.getPosition() != LegendPosition.RIGHT_OF_CHART
						&& this.mLegend.getPosition() != LegendPosition.RIGHT_OF_CHART_CENTER) {
					if (this.mLegend.getPosition() != LegendPosition.LEFT_OF_CHART
							&& this.mLegend.getPosition() != LegendPosition.LEFT_OF_CHART_CENTER) {
						if (this.mLegend.getPosition() == LegendPosition.BELOW_CHART_LEFT
								|| this.mLegend.getPosition() == LegendPosition.BELOW_CHART_RIGHT
								|| this.mLegend.getPosition() == LegendPosition.BELOW_CHART_CENTER) {
			...
			this.mViewPortHandler.restrainViewPort(Math.max(min, offsetLeft), Math.max(min / 1.5F, offsetTop),
	...
	protected float[] getMarkerPosition(Entry e, int dataSetIndex) {
		...
		this.getTransformer(
				((BarLineScatterCandleDataSet) ((BarLineScatterCandleData) this.mData).getDataSetByIndex(dataSetIndex))
						.getAxisDependency()).pointValuesToPixel(pts);
		return pts;
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		return this.mListener != null && !this.mDataNotSet ? (!this.mTouchEnabled ? false : this.mListener.onTouch(
				this, event)) : false;
	}
	...
	public BarLineScatterCandleDataSet<? extends Entry> getDataSetByTouchPoint(float x, float y) {
		...
		return h != null ? (BarLineScatterCandleDataSet) ((BarLineScatterCandleData) this.mData).getDataSetByIndex(h
				.getDataSetIndex()) : null;
	}
	...
	public int getHighestVisibleXIndex() {
		...
		this.getTransformer(AxisDependency.LEFT).pixelsToValue(pts);
		return pts[0] >= (float) ((BarLineScatterCandleData) this.mData).getXValCount() ? ((BarLineScatterCandleData) this.mData)
				.getXValCount() - 1 : (int) pts[0];
	}
