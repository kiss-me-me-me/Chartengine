...
public abstract class ChartData<T extends DataSet<? extends Entry>> {
	...
	private void isLegal(List<? extends DataSet<?>> dataSets) {
		if (dataSets != null) {
			for (int i = 0; i < dataSets.size(); ++i) {
				if (((DataSet) dataSets.get(i)).getYVals().size() > this.mXVals.size()) {
	...
	public float getYMin(AxisDependency axis) {
		return axis == AxisDependency.LEFT ? this.mLeftAxisMin : this.mRightAxisMin;
	}
	...
	public float getYMax(AxisDependency axis) {
		return axis == AxisDependency.LEFT ? this.mLeftAxisMax : this.mRightAxisMax;
	}
	...
	public T getDataSetByLabel(String label, boolean ignorecase) {
		...
		return (T) (index >= 0 && index < this.mDataSets.size() ? (DataSet) this.mDataSets.get(index) : null);
	}

	public T getDataSetByIndex(int index) {
		return (T) (this.mDataSets != null && index >= 0 && index < this.mDataSets.size() ? (DataSet) this.mDataSets
				.get(index) : null);
	}
